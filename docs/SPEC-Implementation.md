...
```json
{
  ...
  "execution": {
    "mode": "workflow",
    "workflow": { "repo": "owner/name", "workflow_id": "spell.yml", "dispatch": "repository_dispatch" }
  },
...
```ts
export async function cast(spellId: string, input: any, opt: { mode?: string; idempotencyKey: string }) {
