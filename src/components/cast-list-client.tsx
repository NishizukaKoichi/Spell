  ...
  const filteredCasts = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (cast) =>
          cast.spell.name.toLowerCase().includes(query) || cast.id.toLowerCase().includes(query)
      );
    }
    ...
    return (
      <Card className="border-white/10">
        <CardContent className="flex flex-col items-center justify-center py-16">
          <p className="text-lg text-white/60 mb-4">No casts yet. Start by casting a spell!</p>
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right text-sm text-white/60">
                      ...
                      {cast.duration && <p className="mt-1">{cast.duration}ms duration</p>}
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-mono">${(cast.costCents / 100).toFixed(2)}</span>
                    </div>
                    {cast.artifactUrl && (
                      <div>
                    ...
                    {cast.errorMessage && (
                      <div className="col-span-2">
                        ...
                        <span className="ml-2 text-red-400">{cast.errorMessage}</span>
                      </div>
                    )}
                  </div>
                </CardContent>
