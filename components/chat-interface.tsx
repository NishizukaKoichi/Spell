  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <div className="flex flex-1 flex-col">
        <header className="sticky top-0 z-40 flex items-center gap-2 bg-background px-3 py-2 sm:px-4 sm:py-3">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-9 w-9 text-foreground hover:bg-foreground hover:text-background sm:h-10 sm:w-10"
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-9 w-9 text-foreground hover:bg-foreground hover:text-background sm:h-10 sm:w-10 disabled:opacity-30"
                >
      ...
      {/* Execution preview - keeping original styling as requested */}
      {showPreview && executionResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
          <div className="relative h-[90vh] w-[90vw] max-w-6xl border-2 border-white bg-black p-6">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-semibold text-white">{executionResult.spellName}</h2>
                <p className="text-sm text-white/60">
                  実行日時: {new Date(executionResult.timestamp).toLocaleString()}
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            <ScrollArea className="h-[calc(100%-80px)]">
              <div className="space-y-4 text-white">
                <div className="border border-white/20 bg-white/5 p-4">
                  <h3 className="mb-2 text-lg font-semibold">レスポンス</h3>
                  <p className="text-white/80">{executionResult.response}</p>
                </div>
                {executionResult.ui && (
                  <div className="border border-white/20 bg-white/5 p-4">
                    <h3 className="mb-2 text-lg font-semibold">UI出力</h3>
                    {executionResult.ui}
                  </div>
                )}
              </div>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent className="bg-background border-2 border-foreground text-foreground">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-foreground">スペル実行の確認</AlertDialogTitle>
            <AlertDialogDescription className="text-muted-foreground">
              このスペルは {selectedSpell?.cost} クレジットかかります。実行しますか？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={handleCancelExecute}
              className="bg-background text-foreground border-foreground hover:bg-foreground hover:text-background"
            >
              いいえ
            </AlertDialogCancel>
            <AlertDialogAction
