    ...
    <div className="bg-background p-3 sm:p-4">
      <form onSubmit={handleSubmit} className="mx-auto max-w-3xl">
        <div className="flex items-center gap-2 sm:gap-3">
          <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger asChild>
              <Button
                type="button"
                size="icon"
                className="h-10 w-10 shrink-0 bg-background text-foreground hover:bg-foreground hover:text-background sm:h-12 sm:w-12"
                onClick={() => {
                  setIsOpen(!isOpen)
                  setSelectedFolder(null)
                }}
            ...
            <PopoverContent className="w-64 p-0 bg-background border-2 border-foreground" align="start">
              <ScrollArea style={{ height: `${getContentHeight()}px` }} className="bg-background">
                <div className="p-2">
                  {!selectedFolder ? (
                    <div className="space-y-1">
                      {grimoire.length === 0 ? (
                        <div className="px-2 py-4 text-center text-sm text-muted-foreground">スペルがありません</div>
                      ) : (
                        grimoire.map((folder) => (
                          <button
                            key={folder.id}
                            onClick={() => handleFolderClick(folder)}
                            className="flex w-full items-center justify-between px-2 py-2 text-sm text-foreground hover:bg-muted"
                          >
                    ...
                    <div className="space-y-1">
                      <button
                        onClick={handleBack}
                        className="flex w-full items-center gap-2 px-2 py-1.5 text-sm font-semibold text-foreground hover:bg-muted"
                      >
                      ...
                      {selectedFolder.bookmarks.length === 0 ? (
                        ...
                        selectedFolder.bookmarks.map((bookmark) => (
                          <button
                            ...
                            className="flex w-full items-center gap-2 px-2 py-2 text-sm text-foreground hover:bg-muted"
                          >
          ...
          <div className="relative flex flex-1 items-center rounded-full border border-foreground bg-background px-3 shadow-sm sm:px-4">
            <Textarea
              value={input}
            ...
            {showSuggestions && filteredSpells.length > 0 && (
              <div className="absolute bottom-full left-0 right-0 mb-2 border-2 border-foreground bg-background shadow-lg z-50">
                <ScrollArea className="max-h-[300px] bg-background text-foreground">
                  <div className="p-2 space-y-1">
                    {filteredSpells.map((spell, index) => (
                      ...
                      >
                        <Bookmark className="h-4 w-4 mt-0.5 shrink-0 text-muted-foreground" />
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-foreground">{spell.name}</div>
                          {spell.description && (
                            <div className="text-xs text-muted-foreground truncate">{spell.description}</div>
                          )}
                          <div className="text-xs text-muted-foreground">by {spell.author}</div>
                        </div>
                      </button>
                    ))}
                  </div>
