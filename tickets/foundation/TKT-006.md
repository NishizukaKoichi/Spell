...
export const ErrorCatalog = {
  ...
  UNAUTHORIZED: (msg = 'Authentication required') =>
    new SpellError('UNAUTHORIZED', msg, 401),

  FORBIDDEN: (msg = 'Insufficient permissions') =>
    new SpellError('FORBIDDEN', msg, 403),
  ...
  BUDGET_CAP_EXCEEDED: (current: number, cap: number, estimate: number) =>
    new SpellError(
      ...
      'Budget cap exceeded',
      402,
      { current_usage: current, cap, estimate, retry_after: 86400 }
    ),
  ...
  WORKFLOW_NOT_FOUND: (workflowId: string) =>
    new SpellError(
      'WORKFLOW_NOT_FOUND',
      `Workflow ${workflowId} not found in repository`,
      404,
  ...
  ARTIFACT_EXPIRED: (runId: string) =>
    new SpellError(
      'ARTIFACT_EXPIRED',
      'Artifact has expired (TTL exceeded)',
      410,
      { run_id: runId }
    ),
  ...
  INTERNAL: (msg = 'Internal server error') =>
    new SpellError('INTERNAL', msg, 500),

  TIMEOUT: (maxSeconds: number) =>
    new SpellError(
      'TIMEOUT',
      `Execution timeout after ${maxSeconds}s`,
      504,
