...
export class BudgetManager {
  async checkBudget(userId: string, estimatedCostCents: number): Promise<BudgetCheckResult> {
    // Get user budget
  ...
  async setCap(userId: string, type: 'monthly' | 'total', capCents: number): Promise<void> {
    const data: any = {};
    if (type === 'monthly') {
...
export async function budgetCheckMiddleware(
  ...
  if (!result.allowed) {
    throw ErrorCatalog.BUDGET_CAP_EXCEEDED(result.current_usage, result.cap, result.estimated_cost);
  }
