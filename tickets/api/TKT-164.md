...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // 7. Budget check
    const budgetResult = await budgetCheckMiddleware(req, auth, estimatedCostCents);
    ...
    await storeIdempotencyResult(req.headers.get('X-Idempotency-Key')!, auth.user_id, response);
...
export class CastOrchestrator {
  ...
  private async executeWorkflow(req: ExecutionRequest): Promise<ExecutionResult> {
    ...
    const run = await executor.execute(req.spell.executionConfig, req.inputs, req.castId);
